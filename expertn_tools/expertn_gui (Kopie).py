...
#f not subprocess.mswindows:
if not platform.system() == 'Windows': 
    locale.setlocale(locale.LC_ALL, 'en_US.UTF-8')
#Hong import gtk, gobject, cairo
...
#Hong from matplotlib.backends.backend_gtkagg import FigureCanvasGTKAgg as FigureCanvas
from matplotlib.backends.backend_gtk3agg import FigureCanvasGTK3Agg as FigureCanvas
#Hong from matplotlib.backends.backend_gtkagg import NavigationToolbar2GTKAgg as NavigationToolbar
...
def do_gui_operation(function, *args, **kw):    
      def idle_func():
          #Hong removed gdk.threads_enter()
          print('2 thread')
          try: 
              function(*args, **kw)
              return False
          finally:
              #Hong removed gdk.threads_leave()
              print('2 thread end')
...
class Popen(subprocess.Popen):
    ...
    if platform.system() == 'Windows': 
        ...
        def send(self, input):
            ...
            try:
                written = os.write(self.stdin.fileno(), input.encode()) #Hong: Type of input/data: bytes required
            ...
            return written #Hong: returns the number of bytes actually written
...
def recv_some(p, t=.1, e=1, tr=5, IfRunning=True):
    #print('4. step')
    ...
    return [b''.join(y),''.join(y2),finish]

def send_all(p, data):
    
    for x in range(len(data)):
    #while len(data):                    #Hong changed: data should be str?       
       # print('10. step')
        sent = p.send(data) #Hong: data: bytes type required; sent = 1
 
        if sent is None:
            raise Exception(message)
        #data = buffer(data, sent)
        memo = memoryview(data.encode()) #Hong: memoyview in stead of buffer
        data= bytes(memo[x:(x+sent)]).decode("utf-8") #Hong: type of bytes required
...
class GtkTabButton(gtk.HBox):
    def __init__(self,Count):
        ...
        #label = gtk.Button.new_with_label(self.label_str)
        self.lab=label
        e = gtk.EventBox()
        e.add(label)
        #Hong changed e.add_events(gtk.gdk.BUTTON_RELEASE_MASK)
        e.add_events(gdk.EventMask.BUTTON_RELEASE_MASK)
        self.closebutton = gtk.Button()
        self.closebutton.set_relief(gtk.ReliefStyle.NONE)
        #self.closebutton.set_relief(gtk.ReliefStyle.NONE)       
        #Hong changed im = gtk.image_new_from_stock("gtk-close",1)
        im = gtk.Image.new_from_stock("gtk-close",1)
        self.closebutton.set_image(im)
        self.pack_start(e,True,False,0)
        #Hong changed self.pack_start(self.closebutton,False,False)
...
class c_XPNStartScreen:
    ...
    def use_png(self):
        ...
        #Hong changed box.set_policy(gtk.POLICY_AUTOMATIC, gtk.POLICY_AUTOMATIC)
        box.set_policy(gtk.PolicyType.AUTOMATIC, gtk.PolicyType.AUTOMATIC)
        #Hong changed image.connect('expose-event', self.on_image_resize2, box,org_width,org_height)
        image.connect('draw', self.on_image_resize2, box,org_width,org_height)
        box.add(image)
    ...
    def use_svg(self):
        ...
        #Hong changed image.connect('expose-event', self.on_image_resize_svg)
        image.connect('draw', self.on_image_resize_svg)
        self.image=image
    ...
    def on_image_resize(self, widget, event, window,org_width,org_height):
        ...
        if self.temp_width != allocation.width:
            ...
            #Hong changed pixbuf = self.pixbuf.scale_simple(allocation.width, allocation.height, gtk.gdk.INTERP_BILINEAR)
...
class MainWindow:
    def __init__(self):
        self.FileAction = []
        builder = gtk.Builder()
        #Hong changed builder.add_from_file("expertn_gui_main.ui")
        builder.add_from_file("expertn_gui_main.ui")
        #builder.add_from_file("expertn_gui_main.glade")
        ...
        self.stopthread = threading.Event() # added by Hong
        self.pausethread =threading.Condition(threading.Lock()) # added by Hong
        self.paused = False # added by Hong
        ...
        # Add file open logic by FH
        self.open_path = os.path.dirname(os.path.abspath(__file__))
        if os.path.isdir(os.path.join(self.open_path,"..","built","cfg")):
            self.open_path = os.path.join(self.open_path,"..","built","cfg")
    ...
    def retrieve_info(self, widget):
        ...
        # added "try" to do only the conversion if the string is not unicode yet. by FH
        try:
            self.project_filename = str(self.project_filename, 'utf-8')
        except:
            pass
        self["notebook1"].set_current_page(1)
        #Hong changed manager = gtk.recent_manager_get_default()
    ...
    def act_output_thread2(self,error):
        ...
        while (1):
            line = error.readline()
            if not line: break
            #Hone removed gdk.threads_enter()
            print('3 thread')
            ...
            #Hone removed gdk.threads_leave()
            print('3 thread end')
    ...
    # added by Hong:
    def output_every_thread(self, p, text, text_error, lastsign,outputlistcount):
       # print('9. step')
        for line in text.split("\n"):
    
            if 'Simulation Day' in line:
                self.labelTime.set_text(line+"\n")
                     
            elif '<TXTOUTPUT:' in line:
                ...
                if self.expertn_run_mode=="stop":
                    send_all(p, "q") # bytes type
                    
                elif self.expertn_run_mode=="pause":
                    #import pdb; pdb.set_trace()
                    #Hong removed gdk.threads_leave()
                    print('4.b thread end')
                    ...
                    print('4.c thread end')
                    
                    while (self.expertn_run_mode=="pause"):
                        print('mode in pause: ',self.expertn_run_mode)
                        nothing=0.1
                    ...
                    print('mode out of pause: ',self.expertn_run_mode) 
                ...
                if (os.name=='posix'):
                    ...
                    #print('pasisert es was1?')
                    self.textviewoutput.scroll_to_iter(self.textoutput.get_end_iter(),0.1, 1, 1,1)
                          
                else:
                    time.sleep(self.delay_workaround)
                    #print('pasisert es was2?')
                    do_gui_operation(self.textoutput.insert, self.textoutput.get_end_iter(),line+"\n")
                    do_gui_operation(self.textviewoutput.scroll_to_iter, self.textoutput.get_end_iter(),0.1, 1,1,1)
    ...
    def act_output_thread(self,shell):
       ...
       # print('2. step')
        ...
        while not self.stopthread.isSet(): # added by Hong to controll the thread by clicking "stop"
        # while (1):        
            try:
                #print('3. step')
                ...
                with self.pausethread: # added by Hong to controll the thread by clicking "pause"
                    #print('in thread')
                    GLib.idle_add(self.output_every_thread, p, text,text_error, lastsign, outputlistcount) 
    ...
    def on_run_expertn(self,widget, data=None):
        if self.expertn_run_mode=="pause":
            ...
            self.paused = False  #Hong added
            self["buttonExpertNStart"].set_sensitive(False) #Hong added
            self["buttonExpertNPause"].set_sensitive(True) #Hong added
            self["buttonExpertNStop"].set_sensitive(True) #Hong added
            self.pausethread.notify() #Hong added
            self.pausethread.release() #Hong added
        ...
        self["buttonExpertNPause"].set_sensitive(True)
        self["buttonExpertNStop"].set_sensitive(True)
        self.stopthread.clear() #Hong added
        #Hong changed _thread.start_new_thread(self.act_output_thread, ([self.expertn_bin,"-d",self.base_path,"-c",self.project_filename,'-n'],))
        thread= threading.Thread(target=self.act_output_thread, args=([self.expertn_bin,"-d",self.base_path,"-c",self.project_filename,'-n'],))        
        thread.start()


    def on_run_valgrind_expertn(self,widget, data=None):
        _#Hong thread.start_new_thread(self.act_output_thread, ("valgrind --leak-check=yes /home/chrikle/projects/newmatrix/expertn_modul/expertn_bin/Debug/expertn_bin",))
        

    def on_stop_expertn(self,widget, data=None):        
        print('stop!!!')
        self.expertn_run_mode="stop"
        self["buttonExpertNStart"].set_sensitive(True) # added by Hong
        self["buttonExpertNPause"].set_sensitive(False) # added by Hong
        self["buttonExpertNStop"].set_sensitive(False) # added by Hong
        self.stopthread.set() # # added by Hong to controll the thread by clicking "stop"
        #  Fuer Linux:
        #if (os.name=='posix'):
    ...
    def on_pause_expertn(self,widget, data=None):
        ...
        self["buttonExpertNPause"].set_sensitive(False)
        self["buttonExpertNStop"].set_sensitive(True)        
        self.paused = True  # added by Hong
        print('pause clicked')
        self.pausethread.acquire() # added by Hong to controll the thread by clicking "pause"
...
##            self['notebook_view'].set_current_page(0)
    ...
    def delete_output_tab(self,widget,data, noteb,source):
        ...
        noteb.connect_object("create-window",self.newWin,noteb,tab.label_str) # Hong: ist hier richtig?
        return False
    ...
    def open_cdb_var_list(self,title,var_list,filename=''):
        ...
        if (val!='Open no file'):
            ...
            tabbtn.show_all()
            #Hong vbox = gtk.VBox(False, 5)
            vbox = gtk.VBox(spacing= 5) # Hong: warum habe ich hier geändert?
            vbox.show()
            ...
            #Hong changed gtk.notebook_set_window_creation_hook(self.newWin,tabbtn.label_str)
            notebookPlot.connect_object("create-window",self.newWin,notebookPlot,tabbtn.label_str)
            ...
            Mwindow = xnoplot.MainWindow(cdb_plot_args)
            Mwindow.window.show()
            print('MainWindow showed else again')
    ...
    def  AssistantChooseFile(self,widget,entryFile,assistant,vbox):
        #Hong changed dialog = gtk.FileChooserDialog("New Expert-N Project",None,gtk.FILE_CHOOSER_ACTION_SAVE,(gtk.STOCK_CANCEL, gtk.RESPONSE_CANCEL,gtk.STOCK_OPEN, gtk.RESPONSE_OK))
        dialog = gtk.FileChooserDialog("New Expert-N Project",None,gtk.FileChooserAction.SAVE,(gtk.STOCK_CANCEL, gtk.ResponseType.CANCEL,gtk.STOCK_OPEN, gtk.ResponseType.OK))
        #dialog = gtk.FileChooserDialog(title="New Expert-N Project",action=gtk.FileChooserAction.SAVE,buttons=(gtk.STOCK_CANCEL, gtk.ResponseType.CANCEL,gtk.STOCK_OPEN, gtk.ResponseType.OK))         
        #Hong test dialog = gtk.FileChooserDialog(title="New Expert-N Project", action=gtk.FileChooserAction.OPEN,)      # --> damit kann man keine neue folder erstellen
        #Hong test dialog.add_buttons(gtk.STOCK_CANCEL, gtk.ResponseType.CANCEL,gtk.STOCK_OPEN, gtk.ResponseType.OK)
        ...
        #Hong dialog.set_default_response(gtk.RESPONSE_OK)
        dialog.set_default_response(gtk.ResponseType.OK)
        response = dialog.run()
        #Hong if response == gtk.RESPONSE_OK:
    ...
    def AssistantChooseTemplate(self,widget,entryFile,assistant,vbox):
        #Hong dialog = gtk.FileChooserDialog("Template Filename",None,gtk.FILE_CHOOSER_ACTION_OPEN,(gtk.STOCK_CANCEL, gtk.RESPONSE_CANCEL,gtk.STOCK_OPEN, gtk.RESPONSE_OK))
        #dialog = gtk.FileChooserDialog("Template Filename",None,gtk.FileChooserAction.OPEN,(gtk.STOCK_CANCEL, gtk.ResponseType.CANCEL,gtk.STOCK_OPEN, gtk.ResponseType.OK))
        dialog = gtk.FileChooserDialog(title="Template Filename", action=gtk.FileChooserAction.OPEN,)        
    ...
    def on_new_project(self,widget, data=None):
        ...
        #Hong assistant.set_page_type(vbox, gtk.ASSISTANT_PAGE_INTRO
        assistant.set_page_type(vbox, gtk.AssistantPageType.INTRO)
        label = gtk.Label("This Assistant will help you to create a new Expert-N Project.b\nThe Assistant is used to guide a user through configuration of an project.")
        #label = gtk.Button.new_with_label("This Assistant will help you to create a new Expert-N Project.\nThe Assistant is used to guide a user through configuration of an project.")
        ...
        #Hong assistant.set_page_type(vbox, gtk.ASSISTANT_PAGE_CONTENT)
        ...
        label = gtk.Label(label="This Assistant will help you to create a new Expert-N Project.\nThe Assistant is used to guide a user through configuration of an project.")
        label.set_line_wrap(True)
        #label = gtk.Button.new_with_label("Please choose the path and the file of your new Expert-N project!")
        #Hong label.set_line_wrap(True) #warum habe ich auskommentiert?
        hbox = gtk.HBox()
        hbox.set_border_width(5)
        entryFile = gtk.Entry()
        button = gtk.Button()
        #Hong button.set_relief(gtk.RELIEF_NONE)
        button.set_relief(gtk.ReliefStyle.NONE)
        button.connect('clicked',self.AssistantChooseFile,entryFile,assistant,vbox)
        #Hong im = gtk.image_new_from_stock("gtk-save-as",1)
        im = gtk.Image.new_from_stock("gtk-save-as",1)
        #im = gtk.Image.new_from_icon_name("gtk-save-as",1) #by FH
        button.set_image(im)
        hbox2 = gtk.HBox()
        ...
        button2 = gtk.Button()
        #Hong button2.set_relief(gtk.RELIEF_NONE)
        button2.set_relief(gtk.ReliefStyle.NONE)
        button2.connect('clicked',self.AssistantChooseTemplate,entryFile2,assistant,vbox)
        #Hong im2 = gtk.image_new_from_stock("gtk-open",1)
        im2 = gtk.Image.new_from_stock("gtk-open",1)
# im2 = gtk.Image.new_from_icon_name("gtk-open",1) # by FH
        ...
        vbox.pack_start(gtk.Button.new_with_label("Project Filename:"), False, False, 0)
        
        vbox.pack_start(hbox, False, False, 0)
        #vbox.pack_start(gtk.Label("Template Filename:"), False, False, 0)
        vbox.pack_start(gtk.Button.new_with_label("Template Filename:"), False, False, 0)
        ...
        #Hong assistant.set_page_type(vbox, gtk.ASSISTANT_PAGE_CONFIRM)
        assistant.set_page_type(vbox, gtk.AssistantPageType.CONFIRM)
        label = gtk.Label("The configuration of your new Expert-N project is done.b\nClick to Apply!")
# label = gtk.Label("The configuration of your new Expert-N project is done.\nClick to Apply!") #by FH        
#label = gtk.Button.new_with_label("The configuration of your new Expert-N project is done.\nClick to Apply!")
        label.set_line_wrap(True)
        vbox.pack_start(label, True, True, 0)
        assistant.set_page_complete(vbox, True)
    ...
    def on_open_project(self,widget, data=None):
        #Hong dialog = gtk.FileChooserDialog("Open Expert-N Project",None,gtk.FILE_CHOOSER_ACTION_OPEN,(gtk.STOCK_CANCEL, gtk.RESPONSE_CANCEL,gtk.STOCK_OPEN, gtk.RESPONSE_OK))
        #dialog = gtk.FileChooserDialog("Open Expert-N Project",None,gtk.FileChooserAction.OPEN,(gtk.STOCK_CANCEL, gtk.ResponseType.CANCEL,gtk.STOCK_OPEN, gtk.ResponseType.OK))
        dialog = gtk.FileChooserDialog(title="Open Expert-N Project", action=gtk.FileChooserAction.OPEN,)    
        dialog.add_buttons(gtk.STOCK_CANCEL, gtk.ResponseType.CANCEL,gtk.STOCK_OPEN, gtk.ResponseType.OK)
        if os.path.isdir(self.open_path):
            dialog.set_current_folder(self.open_path)  # by FH
        filter = gtk.FileFilter()
        filter.set_name("Expert-N Project Files (*.xpn)")
        filter.add_pattern("*.xpn")
        dialog.add_filter(filter)
        ...
        if response == gtk.ResponseType.OK:	
            self.project_filename=dialog.get_filename()
            self["notebook1"].set_current_page(1)
            #Hong manager = gtk.recent_manager_get_default()
            manager = gtk.RecentManager.get_default()
            manager.add_item(convert_file_to_uri(self.project_filename))
            self.window.set_title("Expert N Control Center    -  "+xnoplot.get_filename_without_path(self.project_filename).split(".")[0] +"  -   ")
            self.open_path = dialog.get_current_folder() #by FH
        dialog.destroy()
    ...
    def on_button_New_Template_File_Open(self,widget,data=None):
        ...
        #dialog = gtk.FileChooserDialog("Choose Template File",None,gtk.FileChooserAction.OPEN,(gtk.STOCK_CANCEL, gtk.ResponseType.CANCEL,gtk.STOCK_OPEN, gtk.ResponseType.OK))
        dialog = gtk.FileChooserDialog(title="Choose Template File", action=gtk.FileChooserAction.OPEN,)        
        dialog.add_buttons(gtk.STOCK_CANCEL, gtk.ResponseType.CANCEL,gtk.STOCK_OPEN, gtk.ResponseType.OK)
        dialog.set_current_folder(get_fullpath_from_relative(self.base_path,'cfg_template'))
        filter = gtk.FileFilter()
    ...
    def on_NewGridTemplateFileButton(self,widget,data=None):
        #Honf dialog = gtk.FileChooserDialog("Choose Template File",None,gtk.FILE_CHOOSER_ACTION_OPEN,(gtk.STOCK_CANCEL, gtk.RESPONSE_CANCEL,gtk.STOCK_OPEN, gtk.RESPONSE_OK))
        #dialog = gtk.FileChooserDialog("Choose Template File",None,gtk.FileChooserAction.OPEN,(gtk.STOCK_CANCEL, gtk.ResponseType.CANCEL,gtk.STOCK_OPEN, gtk.ResponseType.OK))
        dialog = gtk.FileChooserDialog(title="Choose Template File", action=gtk.FileChooserAction.OPEN,)        
        dialog.add_buttons(gtk.STOCK_CANCEL, gtk.ResponseType.CANCEL,gtk.STOCK_OPEN, gtk.ResponseType.OK)
        dialog.set_current_folder(get_fullpath_from_relative(self.base_path,'cfg_template'))
    ...
    def dialog_project_options_save(self):
           ...
           # config.set("system","output_database_string",self["entryDatabaseString"].get_text())
            ...
            configfile = open(self.project_filename, 'w')
            config.write(configfile)
    ...
    def on_add_module(self,widget, data=None):
        #Hong dialog = gtk.FileChooserDialog("Add Moduls",None,gtk.FILE_CHOOSER_ACTION_OPEN,(gtk.STOCK_CANCEL, gtk.RESPONSE_CANCEL,gtk.STOCK_OPEN, gtk.RESPONSE_OK))
        #dialog = gtk.FileChooserDialog("Add Moduls",None,gtk.FileChooserAction.OPEN,(gtk.STOCK_CANCEL, gtk.ResponseType.CANCEL,gtk.STOCK_OPEN, gtk.ResponseType.OK))
        dialog = gtk.FileChooserDialog(title="Add Moduls", action=gtk.FileChooserAction.OPEN,)        
        dialog.add_buttons(gtk.STOCK_CANCEL, gtk.ResponseType.CANCEL,gtk.STOCK_OPEN, gtk.ResponseType.OK)
        filter = gtk.FileFilter()
    ...
    def on_result_file_add(self,widget, data=None):
        #Hong dialog = gtk.FileChooserDialog("Add Result File",None,gtk.FILE_CHOOSER_ACTION_OPEN,(gtk.STOCK_CANCEL, gtk.RESPONSE_CANCEL,gtk.STOCK_OPEN, gtk.RESPONSE_OK))
        #dialog = gtk.FileChooserDialog("Add Result File",None,gtk.FileChooserAction.OPEN,(gtk.STOCK_CANCEL, gtk.ResponseType.CANCEL,gtk.STOCK_OPEN, gtk.ResponseType.OK))
        dialog = gtk.FileChooserDialog(title="Add Result File", action=gtk.FileChooserAction.OPEN,)        
        dialog.add_buttons(gtk.STOCK_CANCEL, gtk.ResponseType.CANCEL,gtk.STOCK_OPEN, gtk.ResponseType.OK)
        filter = gtk.FileFilter()
    ...
    def on_buttonSaveDescription(self,widget,textbufferDescription,cfg_name):
        text = textbufferDescription.get_text(textbufferDescription.get_start_iter(),textbufferDescription.get_end_iter(), False).replace("\n","$n$") #Hong added False
        config = ConfigParser()
        config.read(cfg_name)
        ...
        #Hong configfile = open(cfg_name, 'wb')
        configfile = open(cfg_name, 'w') #Hong bytes needed
        config.write(configfile)
        configfile.close()
    ...
    def Create_Instance_Statistic(self,id1,gi,gj):
        ...
        #Hong vboxinstanceStatistic.pack_start(self.instanceStatisticcanvas,True,True)
        vboxinstanceStatistic.pack_start(self.instanceStatisticcanvas,True,True,0)
        if sum_frac!=1.0:
            #self.instanceStatisticLabel=gtk.Label("Error: Sum of Fraction is not 100 %!")
            self.instanceStatisticLabel=gtk.Button.new_with_label("Error: Sum of Fraction is not 100 %!")
        else:
            #self.instanceStatisticLabel=gtk.Label("")
            self.instanceStatisticLabel=gtk.Button.new_with_label("")

        #Hong vboxinstanceStatistic.pack_start(self.instanceStatisticLabel,False,False)
        vboxinstanceStatistic.pack_start(self.instanceStatisticLabel,False,False,0)
        vboxinstanceStatistic.show_all()

        [i1,j1,i2,j2] =  xpn_cfg_manager.get_grid_size(grids,int(id1))
        i = list(range(i1-1,i2+2))
        j = list(range(j1-1,j2+2))
        ...
        #Hong vboxGridStatistic.pack_start(self.GridStatisticccanvas,True,True)
        vboxGridStatistic.pack_start(self.GridStatisticccanvas,True,True,0)
        #self.GridStatisticLabel=gtk.Label("")
        self.GridStatisticLabel=gtk.Button.new_with_label("")

        #Hong vboxGridStatistic.pack_start(self.GridStatisticLabel,False,False)
        vboxGridStatistic.pack_start(self.GridStatisticLabel,False,False,0)
    ...
    def on_cfg_modul_dialog(self,widget, data=None,filechoosen=""):
        ...
        for i in range(len(modules_list)):
                ...
                #Hong pbuf = gdk.pixbuf_new_from_file(modules_list[i].icon
                pbuf = Pixbuf.new_from_file(modules_list[i].icon)               
        ...
        first = model.get_iter_first()
        selection = treeviewModel.get_selection()
        #Hong selection.set_mode(gtk.SELECTION_SINGLE)
    ...
    def Save_Modul_CFG_File(self,ModelsConfgFilename,modules_list,xpn_modules):
         ...
         #Hong configfile = open(ModelsConfgFilename, 'wb')
         configfile = open(ModelsConfgFilename, 'w') # Hong: need bytes
    ...
    def on_change_instances_combo(self,combobox,xpn_modules,modules_list):
        ...
        if iter!=None:
           ...
           while (1):
                ...
                lines = lines.decode('utf_8') #Hong added
    ...
    def on_Model_Selected(self,data, modules_list,xpn_modules):
        ...
        if (iter!=None):
                ...
                #Hong scrolled_window2.set_policy(gtk.POLICY_NEVER, gtk.POLICY_NEVER)#gtk.POLICY_AUTOMATIC, gtk.POLICY_AUTOMATIC)
                scrolled_window2.set_policy(gtk.PolicyType.NEVER, gtk.PolicyType.NEVER)
                #Hong scrolled_window2.add_with_viewport(hbox21)
                scrolled_window2.add(hbox21)
                ...
                for submodules in modules_list[val].submodules:
                    ...
                    #Hong combobox1 = gtk.combo_box_new_text()
                    combobox1 = gtk.ComboBoxText()
                    i5=0
                    ...
                    #Hong OptionButton.set_relief(gtk.RELIEF_NONE)
                    OptionButton.set_relief(gtk.ReliefStyle.NONE)
                    #Hong im = gtk.image_new_from_stock(gtk.STOCK_EDIT,1)
                    im = gtk.Image.new_from_icon_name(gtk.STOCK_EDIT, 1)
                    ...
                    #Hong HelpButton.set_relief(gtk.RELIEF_NONE)
                    HelpButton.set_relief(gtk.ReliefStyle.NONE)
                    #Hong im = gtk.image_new_from_stock(gtk.STOCK_HELP,1)
                    im = gtk.Image.new_from_icon_name(gtk.STOCK_HELP,1)
                    HelpButton.add(im)
                    ...
                    #Hong hbox5.pack_end(HelpButton,False,False)
                    hbox5.pack_end(HelpButton,False,False,0)
                    #Hong hbox5.pack_end(OptionButton,False,False)
                    hbox5.pack_end(OptionButton,False,False,0)
                    #Hong vbox1.pack_start(hbox5,False,False)
    ...
    def on_submodel_change(self,combobox,OptionButton,xpn_module_list):
        ...
        if item.description=='':
            ...
            textlist = item.description.split(b"\\n") # ? if b needed ?
            text = ""
            for txt in textlist:
                text = text + b'\n' + txt #? if b needed ?
        ...
        for param in item.params:
            #text2=gtk.Label()
            text2=gtk.Button.new_with_label()
            S = '<b>'+param+':</b>'
            text2.set_markup(S)
            textentry1 = gtk.Entry()
            textentry1.set_text(item.params_act_val[i])
            #text3=gtk.Label()
            text3=gtk.Button.new_with_label()
        ...
        if len(item.params)!=0:
            #text5=gtk.Label()
            text5=gtk.Button.new_with_label()
            text5.set_markup('<span foreground="blue"><b>Options:</b> </span>')
            #text4=gtk.Label()
            text4=gtk.Button.new_with_label()
            text4.set_markup("")
            #text3=gtk.Label()
            text3=gtk.Button.new_with_label()
            text3.set_markup("")
    ...
    def on_cfg_dialog_SystemSettings(self,widget, data=None):
       ...
       if (id==1):
           #Hong configfile = open("expertn_gui.ini", 'wb')
           configfile = open("expertn_gui.ini", 'w') #Hong need bytes
    ...
    def on_ToolsMenueActivated(self,widget,data=None):
        ...
        #FH simple change -> no allocation needed in gtk3, and menu.popup got an additional 0 which is the parameter activate_time, but I have no clue what that means, but it seems to work
        ...
        menu.popup(None, None, None, 1,0,0)




#gobject.threads_init() #Hong removed
#if (os.name=='posix'): #Hong removed
#    gdk.threads_init()
...
if __name__ == "__main__":
    ...
    Mwindow = MainWindow()
    Mwindow.window.show()
    #Hong removed gtk.gdk.threads_enter
    #Hong gdk.threads_enter()
    #print('1. thread bigins')
    #Hong removed gtk.threads_init()
    gtk.main()
    #Hong removed gdk.threads_leave()
    #print('1. thread ends')
