...
from multiprocessing import Process # Hong 20210115
...
class Worker(threading.Thread):
    ...
    def run(self):
        if not self.cmd:
            ...
            print("I cannot find execs")
            GLib.idle_add(self.callback)
        else:
            msg,err =  _process(self.cmd)            
            GLib.idle_add(self.callback,[msg,err])

def _process(shell):
    #proc = psutil.Popen(shell, stdout=PIPE, stderr=PIPE)
    proc = Popen(shell,stdin=PIPE, stdout=PIPE, stderr=PIPE)
    #print('NAME: %s PID = %s' % (proc.name(), str(proc.pid).split()[0]))
    print("_process")
    [msg,err,finish] = recv_some(proc)
...
class MainWindow:
    ...
    def on_run_expertn(self,widget, data=None):
        ...
        #cmd = [self.expertn_bin,"-d",self.base_path,"-c",self.project_filename,'-n']
        #thread= Worker(cmd,callback=self.act_output_thread([self.expertn_bin,"-d",self.base_path,"-c",self.project_filename,'-n'])) 
        ...
        pro = Process(target=self.act_output_thread, args=([self.expertn_bin,"-d",self.base_path,"-c",self.project_filename,'-n'],)) 
        pro.start()
        pro.join()


    def on_run_valgrind_expertn(self,widget, data=None):
        _thread.start_new_thread(self.act_output_thread, ("valgrind --leak-check=yes /home/chrikle/projects/newmatrix/expertn_modul/expertn_bin/Debug/expertn_bin",))
...
#if (os.name=='posix'): #Hong removed 20210115
#    gdk.threads_init()
