...
class c_dialog_statistic_help():
    def __init__(self,builder):
        ...
        #Hong image.connect('expose-event', self.on_image_resize, viewport_image,org_width,org_height)
    ...
    def on_image_resize(self, widget, event, window,org_width,org_height):
        ...
        if self.temp_width != allocation.width:
            ...
            #Hong pixbuf = self.pixbuf.scale_simple(allocation.width, allocation.height, gtk.gdk.INTERP_BILINEAR)
...
class PlotWindow(gtk.VBox):
    def __init__(self,main_class,window,figure_count,filename,liststore,externspreadsheet,open_whole_file=False,x_axis_choice=0,x_axis_choice_name="Time",messfilelist=None):
        ...
        #Hong self.toolbar.set_orientation(gtk.ORIENTATION_HORIZONTAL)
        self.toolbar.set_orientation(gtk.Orientation.HORIZONTAL)
        #Hong self.toolbar.set_style(gtk.TOOLBAR_ICONS)
        ...
        #Hong combobox = gtk.combo_box_new_text()
        ...
        vbox = gtk.VBox()
        #Hong vbox.pack_start(self.toolbar,False,True)
        vbox.pack_start(self.toolbar,False,True,0)
        #Hong self.pack_start(vbox,False,True)
        self.pack_start(vbox,False,True,0)
        #Hong self.pack_start(self.vpaned)
        self.pack_start(self.vpaned, False,True,0)
    ...
    def create_list(self):
        ...
        #Hong scrolled_window.set_policy(gtk.POLICY_AUTOMATIC, gtk.POLICY_AUTOMATIC)
...
#
    ...
    def paintPlot(self,dontautoscale=False):
        ...
        #Hong self.PlotBox.pack_start(self.canvas,True,True)
        self.PlotBox.pack_start(self.canvas,True,True,0)
        #Hong self.PlotBox.pack_start(self.Navtoolbar, False, False)
...
class GtkTabButton(gtk.HBox):
    def __init__(self,Count):
        ...
        e = gtk.EventBox()
        e.add(label)
        #Hong e.add_events(gtk.gdk.BUTTON_RELEASE_MASK)
        e.add_events(gdk.EventMask.BUTTON_RELEASE_MASK)
        self.closebutton = gtk.Button()
        #Hong self.closebutton.set_relief(gtk.RELIEF_NONE)
        self.closebutton.set_relief(gtk.ReliefStyle.NONE)
        #Hong im = gtk.image_new_from_stock("gtk-close",1)
        im = gtk.Image.new_from_stock("gtk-close",1)
        self.closebutton.set_image(im)
        #Hong self.pack_start(e,True,False)
        self.pack_start(e,True,False,0)
        #Hong self.pack_start(self.closebutton,False,False)
...
class MainWindow:
    ...
    def open_plot_window(self,args,filename,open_whole_file=False):
        ...
        #Hong removed!!!1 gtk.notebook_set_window_creation_hook(self.newWin,tabbtn.label_str)
    ...
    def on_toolbutton_refresh(self, widget, data=None):
        ...
        #Hong names=string.split(line,",")
        names=str.split(line,",")
        nr = 0
        for S in names:
    ...
    def SaveXNC(self):
        ...
        treeview_Messure = self["treeview_Messure"]
        model = treeview_Messure.get_model()
        if not model: # this will handle the case when root node is None.
            return
        first = None
        i = 0
        while (1):
