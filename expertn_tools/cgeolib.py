...
def get_files_of_path(path,pathname):
    ...
    os.chdir(old_path)
    output_file_with_path = [path+"/"+x for x in output_files]
...
class xpn_data:
    ...
    def convert_col(self,keys,cmd):
        ...
        for i in indices:
            self.data[1:,i] = [cmd(x) for x in self.data[1:,i]]
    ...
    def get_averaged_data(self, delta, remove_nans=True):
        ...
        for i in range(1,count_for_new_array):
    ...
    def write_csv_file(self, filename, dateformat='%Y-%m-%d %H:%M:%S', spacer=','):
        ...
        f = open(filename, 'w')
        for line in lines:
            f.write(spacer.join([self.__to_str(x, dateformat) for x in line]) + "\n")
        f.close()
        del lines
    ...
    def round_date_and_time(self,roundTo=60*60):
        self.data[1:, 0] = np.array([roundTime(x,roundTo) for x in self.data[1:, 0]])
    ...
    def in1d_date(time1,time2,assume_unique=False, invert=False):
        ...
        """
        ar1 = np.array(list(map(matplotlib.dates.date2num, time1)),dtype=float)
        ar2 = np.array(list(map(matplotlib.dates.date2num, time2)),dtype=float)
    ...
    def get_statistic(arg1, arg2):
        ...
        for var_index in range(1, var_count):
