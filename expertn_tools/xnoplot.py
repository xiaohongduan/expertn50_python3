...
from xpnstatistic3 import xpnstatistic
...
class c_dialog_statistic_help():
    def __init__(self,builder):
        ...
        image.connect('draw', self.on_image_resize, viewport_image,org_width,org_height)
    ...
    #def on_image_resize(self, widget, event, window,org_width,org_height):
        ...
        #dimension = float(org_width)/org_height
        #if self.temp_width != allocation.width and self.temp_height == allocation.height:
            #self.temp_height = allocation.width/dimension
            #self.temp_width = allocation.width
            #print "width", self.temp_width, self.temp_height
            ##allocation.height = self.temp_height
            ##allocation.width = self.temp_width
            #pixbuf = self.pixbuf.scale_simple(self.temp_width, self.temp_height, GdkPixbuf.InterpType.BILINEAR)
            ##GLib.idle_add(widget.set_from_pixbuf, pixbuf)
            ##widget.set_from_pixbuf(pixbuf)
        #if self.temp_height != allocation.height and self.temp_width == allocation.width:
            #self.temp_height = allocation.height
            #self.temp_width = dimension * allocation.height
            #print "height", self.temp_width, self.temp_height
            ##allocation.height = self.temp_height
            ##allocation.width = self.temp_width
            #pixbuf = self.pixbuf.scale_simple(self.temp_width, self.temp_height, GdkPixbuf.InterpType.BILINEAR)
            ###GLib.idle_add(widget.set_from_pixbuf, pixbuf)
            ...
            #if float(allocation.width)/allocation.height > dimension:
                #self.temp_height = allocation.height
                #self.temp_width = dimension * allocation.height
                #print "both1", self.temp_width, self.temp_height
                #self.pixbuf = self.pixbuf.scale_simple(dimension*allocation.height, allocation.height, GdkPixbuf.InterpType.BILINEAR)
                ##widget.set_from_pixbuf(pixbuf)
            #else:
                #self.temp_height = allocation.width/dimension
                #self.temp_width = allocation.width
                #print "both2", self.temp_width, self.temp_height
                #self.pixbuf = self.pixbuf.scale_simple(allocation.width, allocation.width/dimension, GdkPixbuf.InterpType.BILINEAR)
                ##GLib.idle_add(widget.set_from_pixbuf, pixbuf)
                ##widget.set_from_pixbuf(pixbuf)
        #GLib.idle_add(widget.set_from_pixbuf, self.pixbuf)
        
        #if self.temp_width != allocation.width or self.temp_height != allocation.height:
            #self.temp_width = allocation.width
            ...
            ##pixbuf = self.pixbuf.scale_simple(org_width, org_height, GdkPixbuf.InterpType.BILINEAR)
            #widget.set_from_pixbuf(pixbuf)
...
class PlotWindow(Gtk.VBox):
    def __init__(self,main_class,window,figure_count,filename,liststore,externspreadsheet,open_whole_file=False,x_axis_choice=0,x_axis_choice_name="Time",messfilelist=None):
        ...
        # FH 20200926 set cfg folder as standard opening and saving path if XN-structure is standard, otherwise script path will be opened
        ...
        if os.path.isdir(os.path.join(self.save_path,"..","built","cfg")):
            self.save_path = os.path.join(self.save_path,"..","built","cfg")

        #Open Button
        image = Gtk.Image()
        image.set_from_icon_name("document-open",3)
        open_button = Gtk.ToolButton(None)
        open_button.set_icon_widget(image)
        open_button.set_tooltip_text("Open")
        open_button.connect_object("clicked",self.on_open_file,None)
        self.toolbar.insert(open_button,-1)

        #Save Button
        image = Gtk.Image()
        image.set_from_icon_name("document-save-as",3)
        save_button = Gtk.ToolButton(None)
        save_button.set_icon_widget(image)
        save_button.set_tooltip_text("Save as") 
        save_button.connect_object("clicked",self.on_save_file,None)
        self.toolbar.insert(save_button,-1)


        #Edit Button
        image = Gtk.Image()
        image.set_from_icon_name(Gtk.STOCK_EDIT,3)
        edit_button = Gtk.ToolButton(None)
        edit_button.set_icon_widget(image)
        edit_button.set_tooltip_text("Open table of values in spreadsheet application (specify in Preferences)") 
        edit_button.connect_object("clicked",self.on_extern_edit,None)
        self.toolbar.insert(edit_button,-1)

        self.toolbar.insert(Gtk.SeparatorToolItem(),-1)

        #Second legend:
        image = Gtk.Image()
        image.set_from_icon_name("dialog-information",3)
        self.legend_button = Gtk.ToggleToolButton(None)
        self.legend_button.set_icon_widget(image)
        self.legend_button.set_tooltip_text("Show legend")
    ...
    def on_calc_statistics(self,data):
        ...
        S = ""
        S = textbuffer_statistic.get_text(textbuffer_statistic.get_start_iter(),textbuffer_statistic.get_end_iter(),True)
        ...
        textview_statistic.scroll_to_iter(textbuffer_statistic.get_end_iter(),0.0,True,0.0,0.0)
    ...
    #FH 20200625 added path logic for opening file
    def on_open_file(self,widget):
        filechooser =  Gtk.FileChooserDialog(action=Gtk.FileChooserAction.OPEN,buttons=(Gtk.STOCK_CANCEL, Gtk.ResponseType.CANCEL,Gtk.STOCK_OPEN, Gtk.ResponseType.OK))
        
        if os.path.isdir(self.open_path):
            filechooser.set_current_folder(self.open_path)  
        ...
        filter3.set_name("C Debug File (*.cdb)")
        filter3.add_pattern("*.cdb")
        ...
        if response==Gtk.ResponseType.OK:
            ...
            self.paintPlot()
            self.open_path = filechooser.get_current_folder()
        filechooser.hide()
    ...
    #FH 20200625 added path logic for saving file
    def on_save_file(self,widget):
        filechooser =  Gtk.FileChooserDialog(action=Gtk.FileChooserAction.SAVE,buttons=(Gtk.STOCK_CANCEL, Gtk.ResponseType.CANCEL,Gtk.STOCK_SAVE, Gtk.ResponseType.OK))
        
        if os.path.isdir(self.save_path):
            filechooser.set_current_folder(self.save_path)        
        ...
        if response==Gtk.ResponseType.OK:
            ...
            self.save_path = filechooser.get_current_folder()
        filechooser.hide()
...
#
    ...
    def on_toggle_legend(self,widget):
        ...
        if (widget.get_active()==True):
            widget.set_tooltip_text("Hide legend")
        else:
            widget.set_tooltip_text("Show legend")
    
    
    def on_toggle_list(self,widget):
        if (widget.get_active()==True):
            self.list.show()
            widget.set_tooltip_text("Hide table of values")
        else:
            self.list.hide()
            widget.set_tooltip_text("Show table of values")
    ...
    def paintPlot(self,dontautoscale=False):
        ...
        self.PlotBox.pack_start(self.Navtoolbar, False, False,0)
        if repaint==1:
...
class GtkTabButton(Gtk.HBox):
    def __init__(self,Count):
        ...
        im = Gtk.Image.new_from_icon_name("window-close",1)
...
class MainWindow:
    ...
    def freewindestroy(self,widget,data, noteb,source):
        ...
        noteb.connect_object("create-window",self.newWin,noteb,tab.label_str)
        return False
    ...
    def open_plot_window(self,args,filename,open_whole_file=False):
        ...
        #Gtk.Notebook.signals.create_window(self.newWin,tabbtn.label_str)
        #Gtk.notebook_set_window_creation_hook(self.newWin,tabbtn.label_str)
        self.notebookPlot.connect_object("create-window",self.newWin,self.notebookPlot,tabbtn.label_str)
    ...
    def on_ChangeFigureName(self,notebook,button,PlotWindow,tabbtn):
        if button.button==3:
            ...
            if (id==1):
                ...
                tabbtn.set_comment(self["textbufferFigureComment"].get_text(self["textbufferFigureComment"].get_start_iter(),self["textbufferFigureComment"].get_end_iter(),True))
            dialogFigure.hide()

    def on_filechooser_file_set(self,widget,data=None):
        dialog = Gtk.FileChooserDialog("Select a file ",None,Gtk.FileChooserAction.OPEN,(Gtk.STOCK_CANCEL, Gtk.ResponseType.CANCEL,Gtk.STOCK_OPEN, Gtk.ResponseType.OK))
        self.SetFileFilters(dialog)
        ## FH 20200625 display cfg-folder when opening a file as standard, when a file is chosen, open_path is set to the path of the last file
        if os.path.isdir(self.open_path):
            dialog.set_current_folder(self.open_path)
        ...
        if response == Gtk.ResponseType.OK:
            ...
            self.open_path = dialog.get_current_folder()
        dialog.hide()

    # FH 20200625 do not see where the function is used, maybe in expertn_gui.py
    def on_menu_import_Expert_N_file(self,widget, data=None):
        dialog = Gtk.FileChooserDialog("Open...",None,Gtk.FileChooserAction.OPEN,(Gtk.STOCK_CANCEL, Gtk.ResponseType.CANCEL,Gtk.STOCK_OPEN, Gtk.ResponseType.OK))
        if os.path.isdir(self.open_path):
            dialog.set_current_folder(self.open_path)
        dialog.set_default_response(Gtk.ResponseType.OK)
    ...
    def SaveXNC(self):
        ...
        first = None
        i = 0
        # FH 20200703 not so sure about that, but makes sense to avoid error message at closing
        if model != None:
            while (1):
                if i == 0:
                    first = model.get_iter_first()
    ...
    def on_messurements_add_file(self,widget,data=None):
        dialog = Gtk.FileChooserDialog("Add Messurement File",None,Gtk.FileChooserAction.OPEN,(Gtk.STOCK_CANCEL, Gtk.ResponseType.CANCEL,Gtk.STOCK_OPEN, Gtk.ResponseType.OK))
        if os.path.isdir(self.open_path):
            dialog.set_current_folder(self.open_path)  
        filter = Gtk.FileFilter()
        filter.set_name("Expert-N 5.0 Output File (*.xno)")
    ...
    def __init__(self,xnoplot_argv=[]):
        ...
        # FH 20200926 set cfg folder as standard opening path if XN-structure is standard, otherwise script path will be opened
        self.open_path = os.path.dirname(os.path.abspath(__file__))
        if os.path.isdir(os.path.join(self.open_path,"..","built","cfg")):
            self.open_path = os.path.join(self.open_path,"..","built","cfg")
