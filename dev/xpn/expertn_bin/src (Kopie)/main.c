	...
	for (i=0; i<cfg->grid_len; i++) {
		...
		for (i2=0; i2<cfg->grid[i].instance_len; i2++) {
			...
			if ((i3 >= loop_min) && (i3 < loop_max)) {				
                ...
                //Test of Hong on 29.6.2020
                //if (pXSys->id_Mosaic!=NULL)free(pXSys->id_Mosaic);
                //(pXSys->reg_str_inst!=NULL)free(pXSys->reg_str_inst);
                //if (pXSys->xpn_classes!=NULL)free(pXSys->xpn_classes);
