      ...
      if ((pPl->pDevelop->bPlantGrowth==TRUE)&&(pPl->pDevelop->bMaturity==FALSE))
      {
       ...
       if (pPl->pDevelop->iDayAftEmerg > 0)
		{ 
			...
			printf("pPl->pDevelop->iDayAftEmerg= \n",pPl->pDevelop->iDayAftEmerg);
...
//SG20110909: für Ausgabe in *.rfp
      ...
      */
	  ...
	  //RSLNB  = (SLNBC-SLNB)/1.0;//rate, Test of Hong
...
//SG 20110801: geringeres Absterben der Blätter wg. Messdaten PAK 346 (Kraichgau)
     ...
     // LWLV   = min(WLV-1.E-5, LWLVM+REANOR(ESD-DS,LWLVM)*0.03*WLV); //Test of Hong
	  
      LCLV   = LWLV*CFV;//rate
      LNLV   = min(LWLV,LWLVM)*LNCMIN + (LWLV-min(LWLV,LWLVM))*LNC;//rate
      LCRT   = max(min(CSRT-1.E-4,CSRT-min(CSRTN,CSRT)),0.)/DELT;//rate
	 // LCRT   = max(min(CSRT-1.E-4,CSRT-min(CSRTN,CSRT)),0.)/1.0;//rate Test of Hong
      LWRT   = LCRT/CFV;
...
//-------------------------------------------------------------------------------------
	  ...
	  LWLVM  = (LAIC-min(LAIC,LAIN))/SLA0/1.0;
      //SG 20110801:
	  LWLV   = min(WLV-1.E-5, LWLVM+REANOR(ESD-DS,LWLVM)*0.01*WLV);	  
      //LWLV   = min(WLV-1.E-9, LWLVM+REANOR(ESD-DS,LWLVM)*0.03*WLV); //Test of Hong
      LCLV   = LWLV*CFV;
	 ...
	 // LCRT   = max(min(CSRT-1.E-4,CSRT-min(CSRTN,CSRT)),0.)/1.0; //Test of Hong
      LWRT   = LCRT/CFV;
      LNRT   = LWRT*RNCMIN;
      //LWRT   = 0.0;
      //LCRT   = 0.0;
...
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
									...
									
	{
		...
		if ((self->VERN > -0.1) && (DS < 0.25)) // DSCRIT = 0.25
    		 //NDEM   = INSW(SLNMIN-SLN+1.E-5, min(NUPTX,0.01*NDEMAD), 0.);
			 NDEM   = INSW(SLNMIN-SLN+1.E-5, min(NUPTX,0.01*NDEMAD*0.1), 0.);
		else
    		NDEM   = INSW(SLNMIN-SLN+1.E-5, min(NUPTX, NDEMAD), 0.);
	}
