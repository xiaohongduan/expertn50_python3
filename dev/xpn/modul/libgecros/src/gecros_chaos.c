...
//SG20110909: für Ausgabe in *.rfp
     ...
     //LCRT   = max(min(CSRT-EPSILON,CSRT-min(CSRTN,CSRT)),0.)/DELT;//rate [g(C) m-2 d-1]
      //SG 20191105: per day
      LCRT   = max(min(CSRT-EPSILON,CSRT-min(CSRTN,CSRT)),0.)/1.0;//rate [g(C) m-2 d-1]
      ...
      */
      ...
      //SG 20191108
      RSLNB  = (SLNBC-SLNB)/1.0;//per day [g m-2(leaf) d-1]
...
/*
      ...
      NFIXE  = max(0., APCAN-EPSILON-RM)/NOTNUL(CCFIX)*12./44.;
...
//*/   
      ...
      NDEM   = INSW(SLNMIN-SLN+EPSILON, min(NUPTX,NDEMAD), 0.);
      
   //++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
	 SG20140312:
	 Vorschlag von Joachim Ingwersen zur Simulation der Winterung
    if DS < DSCRIT // DSCRIT = 0.25
    	NDEM   = INSW(SLNMIN-SLN+EPSILON, min(NUPTX,0.01*NDEMAD))
    else
    	NDEM   = INSW(SLNMIN-SLN+EPSILON,  min(NUPTX,NDEMAD))
    //++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
      ...
      //SG20191108
      RNDEMP = (NDEM-NDEMP)/1.0;//per day [g(N) m-2 d-1]
...
//-----------------------------------------------------------------------------------------
     ...
     // xpn->pCh->pCProfile->Resp_root = Resp_root; 
	  ...
	  //SG 20191108
      RDCDTP = (DCDTC-DCDTP)/1.0;//per day [g(C) m-2 d-1]
     ...
     //SG20130304: verhindern, dass alles in Stängel und nichts in Blätter geht:
      ...
      // /*
      ESD    = INSW(DETER, ESDI, 1.);
      LWLVM  = (LAIC-min(LAIC,LAIN))/SLA0/DELT; // g m-2 (soil) d-1
	  //Test of Hong 
	  if (LWLVM >0.0)
	  {
		  int Hong;
    ...
    //LWLV   = min(WLV-EPSILON, LWLVM+REANOR(ESD-DS,LWLVM)*0.01*WLV);  //[g m-2 d-1 ], original: 0.03 d-1 
 	LWLV   = min((WLV-EPSILON)/1.0, LWLVM+REANOR(ESD-DS,LWLVM)*0.03*WLV); //[g m-2 d-1 ]
     LWLV   = min((WLV-EPSILON)/1.0, LWLVM+REANOR(ESD-DS,LWLVM)*0.03*WLV*DELT); //[g m-2 d-1 ], Test of Hong
   ...
   // //LCRT   = max(min(CSRT-EPSILON,CSRT-min(CSRTN,CSRT)),0.)/DELT;//rate [g(C) m-2 d-1 ]
   // //SG 20191105: per day
   // LCRT   = max(min(CSRT-EPSILON,CSRT-min(CSRTN,CSRT)),0.)/1.0; //per day [g(C) m-2 d-1 ]
   // LWRT   = LCRT/CFV; // [g m-2 d-1]
   // LNRT   = LWRT*RNCMIN;//rate [g(N) m-2 d-1]
   // */
     ...
     pGPltN->fNStemAccumR    = (double)RNST;//rate
      ...
      RLAI   =  INSW(RWLV, max(-LAIC+(double)EPSILON,SLA0*RWLV), SLA0*RWLV);//rate
...
//-------------------------------------------------------------------------------------
      ...
      //SG 20191108
      //RDCDTP = (DCDTC-DCDTP)/1.0; // per day
      IFSH   = LIMIT(0.,1.,DCST/NOTNUL(DCDTP));
...
//-------------------------------------------------------------------------------------
      ...
      RRD    = INSW(RD-RDMX, min((RDMX-RD)/DELT,(RWRT+LWRT)/(WRB+KR*(WRT+WRTD))), 0.);//rate
      //SG20191108
      RRD    = INSW(RD-RDMX, min((RDMX-RD)/1.0,(RWRT+LWRT)/(WRB+KR*(WRT+WRTD))), 0.);//per day [cm d-1]
...
//-------------------------------------------------------------------------------------
      ...
      //SG 20110801:
	  //LWLV   = min(WLV-EPSILON, LWLVM+REANOR(ESD-DS,LWLVM)*0.01*WLV); //[g m-2 d-1 ], original 0.03 d-1
      LWLV   = min(WLV-EPSILON, LWLVM+REANOR(ESD-DS,LWLVM)*0.03*WLV); //[g m-2 d-1 ]
	 // LWLV   = min(WLV-EPSILON, LWLVM+REANOR(ESD-DS,LWLVM)*0.03*WLV*DELT); //[g m-2 d-1 ], Test of Hong
      ...
      KCRN   = -log(0.05)/6.3424/CFV/WRB/RDMX;
      CSRTN  = 1./KCRN*log(1.+KCRN*max(0.,(NRT*CFV-CRVR*RNCMIN))/RNCMIN);
      LCRT   = max(min(CSRT-EPSILON,CSRT-min(CSRTN,CSRT)),0.)/DELT;
      //SG20191105: per day
      LCRT   = max(min(CSRT-EPSILON,CSRT-min(CSRTN,CSRT)),0.)/1.0;//per day [g(C) m-2 d-1]
      LWRT   = LCRT/CFV;
      LNRT   = LWRT*RNCMIN;
...
//-------------------------------------------------------------------------------------
	  ...
	  //SG20191108
      RNSUPP = (NSUP-NSUPP)/1.0; // per day [g(N) m-2 d-1]
...
//-------------------------------------------------------------------------------------
	  ...
	  //%** Crop nitrogen demand and uptake (g N m-2 d-1)
      RMRE   = max(min(44./12.*0.218*(NTOT-WSH*LNCMIN-WRT*RNCMIN),APCAN-EPSILON-RMUL), 0.);
      RMN    = min(44./12.*0.218*(1.001*NTOT-WSH*LNCMIN-WRT*RNCMIN),APCAN-EPSILON-RMUL);
      RMN    = max(0., min(APCAN-EPSILON,RMUL) + max(RMN, 0.));
      RM     = max(0., min(APCAN-EPSILON,RMUL) + RMRE);
...
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
				...
				||(!lstrcmp((LPSTR)pPl->pGenotype->acCropCode,(LPSTR)"WR"))) // winter and summer crops --> DB!!!
	{
		...
		if ((fparVERN > -0.1) && (DS < 0.25)) // DSCRIT = 0.25
    		NDEM   = INSW(SLNMIN-SLN+EPSILON, min(NUPTX,0.01*NDEMAD), 0.);
		else
    		NDEM   = INSW(SLNMIN-SLN+EPSILON,  min(NUPTX,NDEMAD), 0.);
	}
    //++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    ...
    if((strcmp(pPl->pGenotype->acCropCode,"WH")==0)||(strcmp(pPl->pGenotype->acCropCode,"BA")==0)||(strcmp(pPl->pGenotype->acCropCode,"RP")==0)) // here we have to distinguish between winter and summer crops --> DB!!!
	{
		...
		if ((self->VERN > -0.1) && (DS < 0.25)) // DSCRIT = 0.25
    		NDEM   = INSW(SLNMIN-SLN+EPSILON, min(NUPTX,0.01*NDEMAD), 0.);//original
			//NDEM   = INSW(SLNMIN-SLN+EPSILON, min(NUPTX,0.01*NDEMAD)*DELT, 0.);//changed by Hong, NUPTX has the unit (g N m-2 d-1) 
		else
    		NDEM   = INSW(SLNMIN-SLN+EPSILON,  min(NUPTX,NDEMAD), 0.);//original
			//NDEM   = INSW(SLNMIN-SLN+EPSILON,  min(NUPTX,NDEMAD)*DELT, 0.); //changed by Hong, NUPTX has the unit (g N m-2 d-1) 
	}
    //++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
*/
      ...
      //%---output-variables set to zero and five different times of a day(HOUR)
        ...
        *DAPAR  = 0.;
		...
		/* TMAX = (double) self->weather.fTempMax;     //[°C]
		 ...
		 SD1 = 25.0;
         WSUP1 = WSUP*SD1/RD;
		  //WSUP1 = WSUP*25.0/RD;//Test of Hong
              ...
              double *DCDC,double *DCD,double *FLWC)
{
 ...
 //%---total demand for C at the time step considered
      *DCD    = *DCDC + max(0.,DCDR)/DELT;
      //SG20191108
      *DCD    = *DCDC + max(0.,DCDR)/1.0; // per day [g m-2 d-1]
              ...
              double LNRT,double *RNRT,double *RNST,double *RNLV,double *RTNLV,double *RNSO)
{
 ...
 double NRTN,GNRT;
      ...
      NSHN   = FNSH * NUPT;  // [g(N) m-2 d-1]

      //%---leaf N (NLVA) or root N (NRTA) available for remobilization
/*      NLVA   = INSW(LNCMIN-LNC, NLV-WLV*LNCMIN, 0.) / DELT; // [g(N) m-2 d-1]
      NRTA   = INSW(RNCMIN-RNC, NRT-WRT*RNCMIN, 0.) / DELT;  // [g(N) m-2 d-1]
      ...
      *RNST   = RWST * INSW(-NTA,STEMNC,0.);  // [g(N) m-2 d-1]
      ...
      NGS    = NSHN - *RNST - ENSNC*RWSO;  // [g(N) m-2 d-1]
