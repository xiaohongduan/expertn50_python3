    ...
    if ((pPl->pDevelop->bPlantGrowth==TRUE)&&(pPl->pDevelop->bMaturity==FALSE))
    {
     ...
     if (pPl->pDevelop->iDayAftEmerg > 0)
     {
      OrganSenescence_GECROS(self);
			...
			pCh->pCProfile->fCLitterSurf += (double)10.0*pGPltC->fCLeafLossR*DELT; //Hong on 20191104, x 10.0: [g C m-2 d-1] -> [kg C ha d-1]
			pCh->pCProfile->fNLitterSurf += (double)10.0*pGPltN->fNLeafLossR*DELT; ////Hong on 20191104, x 10.0: [g C m-2 d-1] -> [kg C ha d-1]
            
			pCh->pCProfile->fCLeafLitterSurf += (double)10.0*pGPltC->fCLeafLossR*DELT; //Hong for century_n
			pCB->dCInputSurf += (double)10.0*pGPltC->fCLeafLossR*DELT; //Hong added on 20180731 for C-balance
			...
			//if(pPl->pDevelop->fStageSUCROS>=pPl->pGenotype->fBeginSenesDvs)
			{
				...
				for (L=1;L<=L1;L++)
				{
					...
					if(TRLDF > (double)0.0)
					{
						...
						pCL->fCLitter += (double)10.0*fDeadRootC * pLR->fLengthDensFac/TRLDF*DELT;//Hong on 20191104, x 10.0: [g C m-2 d-1] -> [kg C ha d-1]
						pCL->fNLitter += (double)10.0*fDeadRootN * pLR->fLengthDensFac/TRLDF*DELT; //Hong on 20191104, x 10.0: [g C m-2 d-1] -> [kg C ha d-1]
						
						pCB->dCInputProfile += (double)10.0*fDeadRootC * pLR->fLengthDensFac/TRLDF*DELT; //Hong added on 20180731 for C-balance
						//pCB->dCInputCum += fDeadRootC * pLR->fLengthDensFac/TRLDF*DELT; 
						//Hong 20190507: balance for 0-30 cm profile:	
	                    fCumDepth +=(double)0.1*pSL->fThickness; //cm
	                   if (fCumDepth <=30.0)
	                   {
						   pCB->dCInputProfile_30 += (double)10.0*fDeadRootC * pLR->fLengthDensFac/TRLDF*DELT;
					   }
...
int gecros_NitrogenDemand(gecros *self)
  {
    ...
    if ((pPl->pDevelop->bPlantGrowth==TRUE)&&(pPl->pDevelop->bMaturity==FALSE))
    {
	 ...
	 if (pPl->pDevelop->iDayAftEmerg > 0)
	 {
      //Test of Hong NitrogenFixation_GECROS(self);
     //Test of Hong PotentialNitrogenUptake_GECROS(self);
	 }
    }
...
//AP20160616
	  self->NLVI   = min(self->FNRSH * self->NPL * self->SEEDW * self->EG * self->SEEDNC, self->LNCI * self->CLVI/self->CFV); //[ g N m-2]
...
//End of Hong      
     ...
     //Test of Hong pPltN->fLeafCont      = (double)self->NLVI;
	  pPltN->fLeafCont      = (double)self->NLVI*10.0; //[g/m2] --> [kg/ha]
...
//*/
	...
	//End of Hong
	 ...
	 // pPltB->fStovWeight     += pPltB->fDeadLeafWeight;
      ...
      NLV    = (double)pPltN->fLeafCont/10.0; //[g/m2]<--[kg/ha], Hong on 20191104
...
//    LWLV   = min(WLV-1.E-5, LWLVM+REANOR(ESD-DS,LWLVM)*0.03*WLV);
      ...
      */
      ...
      pPltN->fLeafCont = (double)NLV*10.0; //[g/m2]-->[kg/ha], Hong on 20191104
...
//End of Hong
      ...
      //%-- Nitrogen partitioning between shoots and roots
      HNCCR  = LNCI*exp(-0.4*DS);
	  //Test NDEMD  = INSW(DS-1., WSH*(HNCCR-HNC)*(1.+NRT/NSH)/DELT, 0.);//original, not correct!!! NDEMD is not a rate.
...
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
									...
									
	{
		...
		if ((self->VERN > -0.1) && (DS < 0.25)) // DSCRIT = 0.25
    		 //NDEM   = INSW(SLNMIN-SLN+1.E-5, min(NUPTX,0.01*NDEMAD), 0.);
			NDEM   = INSW(SLNMIN-SLN+1.E-5, min(NUPTX,0.005*NDEMAD), 0.);
		else
    		NDEM   = INSW(SLNMIN-SLN+1.E-5, min(NUPTX,NDEMAD), 0.);
	}
