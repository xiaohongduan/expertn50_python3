...
//SG20110909: für Ausgabe in *.rfp
     ...
     // RSLNB = RSLNB*DELT; //Test of Hong, kein Einfluss
      ///*
...
//-----------------------------------------------------------------------------------------
	 ...
	 // RDCDTP = RDCDTP*DELT; //Test of Hong, --> mehr C
...
//    LWLV   = min(WLV-1.E-5, LWLVM+REANOR(ESD-DS,LWLVM)*0.03*WLV);
      ...
      LCRT   = max(min(CSRT-1.E-4,CSRT-min(CSRTN,CSRT)),0.)/DELT;//rate
	  //LCRT=LCRT*DELT; //Test of Hong, kein Einfluß
	   if (LCRT >EPSILON)
	  {
		  int Hong;
		  Hong=1;
...
//-------------------------------------------------------------------------------------
	 ...
	 // NDEMD =NDEMD*DELT; //Test of Hong
      NDEMA  = CRT * SHSA*SHSA/NOTNUL(DERI);
	 // NDEMA =EPSILON; //Test of Hong
	  //printf("NDEMA= %f\n", NDEMA);
	  //printf("NDEMD= %f\n", NDEMD);
      NDEMAD = INSW(LNC-1.5*LNCI, max(NDEMA, NDEMD), 0.);
	  // NDEMAD = NDEMAD*0.5; // Test of Hong 
      NDEM   = INSW(SLNMIN-SLN+1.E-5, min(NUPTX,NDEMAD), 0.);
	  if ((DS>0.248)&&(DS<0.25))//Test of Hong 		 
	    {
		printf("DS<0.25\n");
        printf("WSH =%f\n",WSH);		
		printf("HNCCR =%f\n",HNCCR);
		...
		printf("NDEM =%f\n",NDEM);
		 ...
		 if ((DS>0.255)&&(DS<0.26))//Test of Hong 		 
	    {
		printf("DS=0.25\n");	
		printf("WSH =%f\n",WSH);		
		printf("HNCCR =%f\n",HNCCR);
		...
		printf("NDEM =%f\n",NDEM);
		 ...
		 if ((DS>0.995)&&(DS<1.0))//Test of Hong 		 
	    {
		printf("DS< 1.0\n");	
		printf("WSH =%f\n",WSH);		
		printf("HNCCR =%f\n",HNCCR);
...
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    ...
    //++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
	if ((dayTime <=self->SUNRIS)||(dayTime>=self->SUNSET)) //Test of Hong
				NDEM=0.0;
...
///*
	{
      ...
      //TEst of Hong
	  if (pPl->pPltWater->fActTranspdt <=0.0)
	  {
