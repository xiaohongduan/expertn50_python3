...
//-----------------------------------------------------------------------------------------
   ...
   // //SG 20191105: per day
   // LCRT   = max(min(CSRT-1.E-4,CSRT-min(CSRTN,CSRT)),0.)/1.0; //per day [g(C) m-2 d-1 ]
...
//-------------------------------------------------------------------------------------
	  ...
	  RLAI   =  INSW(RWLV, max(-LAIC+(double)1.E-5,SLA0*RWLV), SLA0*RWLV); //Test of Hong
	 // if((strcmp(pPl->pGenotype->acCropCode,"WH")==0)||(strcmp(pPl->pGenotype->acCropCode,"BA")==0))
	  if ((LAIC > (double)1)&&(DS < (double)0.5))
		  RLAI   = RLAI *self->DAYL/24.0;
...
//End of Hong	
	...
	/*  if ((int)NewDay(pTi))
	  ...
	  NDEMD = self->NDEM_of_day;*/
	   NDEMD = INSW(DS-1.0, WSH*(HNCCR-HNC)*(1.+NRT/NSH), 0.);
	 // NDEMD = INSW(DS-1.0, WSH*(HNCCR-HNC)*(1.+NRT/NSH)/DELT*self->DAYL/24.0, 0.); //Test of Hong
