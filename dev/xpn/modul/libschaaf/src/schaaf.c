...
int read_irrigation(schaaf* self)
{
    ...
    GKeyFileFlags flags;
    GError* error = NULL;
	char **irr_per_start, **irr_per_end;
	GDate **irrigation_period_start;
	GDate **irrigation_period_end;
	GDate irrigation_period_loop_date;
	double *irrigation_period_amount_day;
	double *irr_per_start_xpn_time, *irr_per_end_xpn_time;
	double irr_per_loop_xpn_time;
	int *irrigation_frequency;
	int irrigation_period_true;
	int irrigation_period_start_len, irrigation_period_end_len, irrigation_period_amount_day_len,irrigation_frequency_len;
	int irrigation_period_len,irr_per_loop_year;
	int *irr_per_start_year, *irr_per_end_year;
    const char* filename;
    char* ini_filename;
    int i, j;
    ...
    if (ini_filename == NULL) {
        ...
        if (S2 != NULL) {
			...
			irrigation_period_true = 0;
			irrigation_period_len = 0;
			irrigation_period_start_len = 0;
			GET_INI_INT(irrigation_period_true,"irrigation_period","irrigation_period_true");
			if (irrigation_period_true != 0)
			{
				GET_INI_STRING_ARRAY_OPTIONAL(irr_per_start,irrigation_period_start_len,1,"9999-09-09","irrigation_period","irrigation_period_starts");
				GET_INI_STRING_ARRAY_OPTIONAL(irr_per_end,irrigation_period_end_len,1,"9999-09-09","irrigation_period","irrigation_period_ends");
				GET_INI_DOUBLE_ARRAY_OPTIONAL(irrigation_period_amount_day,irrigation_period_amount_day_len,1,0.0,"irrigation_period","irrigation_amount");
				GET_INI_INT_ARRAY_OPTIONAL(irrigation_frequency,irrigation_frequency_len,1,1,"irrigation_period","irrigation_frequency");
				CHECK_LEN(irrigation_period_start_len,irrigation_period_end_len);
				CHECK_LEN(irrigation_period_start_len,irrigation_period_amount_day_len);
				CHECK_LEN(irrigation_period_start_len,irrigation_frequency_len);
			
			

				irrigation_period_start = g_malloc0(sizeof(GDate*) * irrigation_period_start_len);
				irrigation_period_end = g_malloc0(sizeof(GDate*) * irrigation_period_end_len);
				irr_per_start_year = g_malloc0(sizeof(int*) * irrigation_period_start_len);
				irr_per_start_xpn_time = g_malloc0(sizeof(double*) * irrigation_period_start_len);
				irr_per_end_year = g_malloc0(sizeof(int*) * irrigation_period_end_len);
				irr_per_end_xpn_time = g_malloc0(sizeof(double*) * irrigation_period_end_len);
				
				//printf("%d \n",irrigation_period_start_len);
				for(i=0; i < irrigation_period_start_len; i++)
				{
					deleteSpaceBegEnd(irr_per_start[i]);
					//printf("%s \n",irr_per_start[i]);
					irrigation_period_start[i] = convert_str_to_gdate(irr_per_start[i]);
					xpn_time_conv_gdate_to_xpn_time(irrigation_period_start[i],&(irr_per_start_year[i]), &(irr_per_start_xpn_time[i])); 
					deleteSpaceBegEnd(irr_per_end[i]);
					irrigation_period_end[i] = convert_str_to_gdate(irr_per_end[i]);
					xpn_time_conv_gdate_to_xpn_time(irrigation_period_end[i],&(irr_per_end_year[i]), &(irr_per_end_xpn_time[i])); 
					irrigation_period_len += (int)((xpn_time_sub_time(irr_per_end_year[i],irr_per_end_xpn_time[i],irr_per_start_year[i],irr_per_start_xpn_time[i]) +1)/irrigation_frequency[i]);
					//printf("%d %f %d %f \n",irr_per_end_year[i],irr_per_end_xpn_time[i],irr_per_start_year[i],irr_per_start_xpn_time[i]);
				}
				g_free(irrigation_period_start);
				g_free(irrigation_period_end);
			
			}
			
            self->irr_gdate = g_malloc0(sizeof(GDate*) * (self->irr_date_len + irrigation_period_len));
			...
			//printf("%d %d %d\n",self->irr_date_len + irrigation_period_start_len,self->irr_date_len,irrigation_period_start_len);

            for (i = 0; i < self->irr_date_len + irrigation_period_start_len; i++) {
                if (i < self->irr_date_len)
				{
				...
				if ((i >= self->irr_date_len) && (irrigation_period_true != 0))
				{
					j = i - self->irr_date_len;
					irr_per_loop_xpn_time = irr_per_start_xpn_time[j];
					irr_per_loop_year = irr_per_start_year [j];
					//printf("%d %d %f %d %f \n",j,irr_per_loop_year,irr_per_loop_xpn_time,irr_per_start_year [j],irr_per_start_xpn_time[j]);
					while(((irr_per_loop_xpn_time <= irr_per_end_xpn_time[j]) && (irr_per_loop_year == irr_per_end_year[j]))
						||(irr_per_loop_year < irr_per_end_year[j]))
					{
						xpn_time_conv_xpn_time_to_gdate(&irrigation_period_loop_date,irr_per_loop_year,irr_per_loop_xpn_time);
						//printf("%d %f \n",irr_per_loop_year,irr_per_loop_xpn_time);
						irr = g_malloc0_n(1, sizeof(STIRRIGATION));
						irr->Day = g_date_get_day(&irrigation_period_loop_date);
						irr->Month = g_date_get_month(&irrigation_period_loop_date);
						irr->Year = g_date_get_year(&irrigation_period_loop_date);
						irr->fAmount = irrigation_period_amount_day[j];
						//printf("%d %d %d %f \n",irr->Day,irr->Month,irr->Year,irr->fAmount);
						xpn_time_add_dt(&(irr_per_loop_year), &(irr_per_loop_xpn_time),(double)irrigation_frequency[j]);
						if (i == 0) {
							irr_first = irr;
							irr_act = irr;
							} else {
...
/*			for(irr_act = irr_first; irr_act->pNext != NULL; irr_act = irr_act->pNext )
			{
				printf("%d %d %d %f \n", irr_act->Day, irr_act->Month, irr_act->Year, irr_act->fAmount);
			}*/
			...
			if(irrigation_period_true != 0)
			{
					g_free(irr_per_start_year);
					g_free(irr_per_end_year);
					g_free(irr_per_start_xpn_time);
					g_free(irr_per_end_xpn_time);
					g_free(irrigation_period_amount_day);
					g_strfreev(irr_per_start);
					g_strfreev(irr_per_end);
			}			
			
			g_free(self->irr_gdate);
